"""Appends multipage CSS extensions to core.css."""
...
/* Breadcrumb */
.breadcrumb{padding:14px 0 0;max-width:1280px;margin:0 auto;padding-left:32px;padding-right:32px;display:flex;align-items:center;flex-wrap:wrap;gap:4px;font-family:var(--mono);font-size:10px;}
...
.cp-order{background:var(--bg3);border:1px solid var(--brd);border-radius:var(--rl);padding:24px;position:sticky;top:80px;overflow:hidden;position:relative;}
...
.cart-qty-btn{width:28px;height:28px;background:var(--bg4);border:1px solid var(--brd);border-radius:4px;cursor:pointer;font-size:14px;display:flex;align-items:center;justify-content:center;color:var(--t2);font-family:var(--mono);transition:all .15s;min-height:44px;min-width:44px;}
...
.cart-remove{background:none;border:none;color:var(--t3);cursor:pointer;font-size:16px;padding:6px;transition:color .15s;margin-left:4px;min-height:44px;min-width:44px;}
...
.co-form-col .co-summary{position:sticky;top:80px;}
...
/* Calculator (new multipage layout) */
...
p = Path(__file__).parent / 'assets' / 'css' / 'core.css'
with open(p, 'a', encoding='utf-8') as f:
    f.write(CSS_EXT)

print("CSS extensions appended successfully.")
